...
import com.example.demo.report.mapper.IDailyReportMapper;
...
public class DailyReportService {
    ...
    public void sendDailyReport(DailyReport report){
        ...
        try {
            ...
            message.setSubject("Daily device report – " + report.getReportDate());
            message.setContent(html, "text/html; charset=UTF-8");
            javaMailSender.send(message);
        } catch (MessagingException e){
            throw new GeneralException("Failed to send report email", e);
        }
    }
}
